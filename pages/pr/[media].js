...
function News({ allData }) {
  ...
  // Media query: Check if the viewport width is greater than 768px
  const isDesktop = useMediaQuery(768);

  // Ensure allData.recent exists before filtering
  ...
  return (
    <Wrapper title={allData?.metaTitle || ''} description={allData?.metaDescription || ''} image={allData?.image ||null}>
      {/* Search Section */}
      <div className='media-container-individual'>
        ...
        <div className="media-sidebar">
          {filteredPosts.length > 0 ? (
            filteredPosts.map((post) => (
              <div key={post.id} className="media-sidebar-item">
                {post.image && <img src={post.image} alt={post.title} className="media-sidebar-img" />}
                <div className="media-sidebar-content">
                  <p className="media-sidebar-date">{moment(post.date).format('MMM YYYY')} | 8 Mins Read</p>
                  <h4 className="media-sidebar-title">{post.title}</h4>
                </div>
              </div>
            ))
          ) : (
          ...
          {allData?.related?.length > 0 ? (
            allData.related.map((post) => (
              <a
                ...
                className="blog-item blogs-media-item"
              >
                {post.image && <img src={post.image} alt={post.name} className="blogs-media-img" />}
                <div>
                  <h4 className="blogs-media-title">{post.name}</h4>
                </div>
              </a>
            ))
          ) : (
      ...
      {/* Conditionally render BottomBar for mobile only */}
      {!isDesktop && <BottomBar />}
    </Wrapper>
  );
}
...
export async function getStaticProps({ params }) {
  try {
    ...
    if (!data?.result?.detail?.length) {
      return { notFound: true }; // Trigger a 404 for invalid slugs
    }
