...
export default function BlogContent({
  ...
  const { before: contentBeforeCTA, after: contentAfterCTA } = insertFormByMarker(html);

  /* ==============================
     Table of Contents
  ============================== */

  const tocItems = [...html.matchAll(/<h([2-4])[^>]*>([\s\S]*?)<\/h\1>/gi)].map(
    ([_, __, inner], i) => {
      ...
      <div className={styles.blogContentWrap}>
        <div className={styles.flexLayout}>
          ...
          <aside className={`${styles.toc} ${tocOpen ? styles.tocOpen : ""}`}>
            <h4 className={styles.tocHead} onClick={() => setTocOpen((v) => !v)}>
          ...
          <aside
            ...
            style={data.type === "news" && isDesktop ? { marginTop: "-215px" } : {}}
          >
