...
import HomePageCta from "../shared/forms/CTANEW.jsx";
import { submitEnquiry } from "../lib/FormApi.jsx";
import SubmitEnquiry from "../lib/FormApi.jsx"; // 👈 Make sure this exports component
...
export default function BlogContent({ detailContent, popular, recent, data, name, slug }) {
  ...
  useEffect(() => {
    window.submitEnquiryForm = (values) => submitEnquiry(values, router, slug);
  ...
  const htmlWithIDs = addHeadingIDs(detailContent?.description || "");

  const { before: contentBeforeCTA, after: contentAfterCTA } = insertFormByMarker(htmlWithIDs);
          ...
          <div className={styles.sidebar}>
            <div className={styles.sdCardNews}>
              ...
              <div className={styles.sdLatestBlogsNews}>
                {recent?.map((blog) => (
                  <Link
                    href={data === "news" ? `/news/${blog.slug}` : `/blog/${blog.slug}`}
                    key={blog.id}
                  >
