...
const normalize = (prop) => {
  ...
  const image =
    (Array.isArray(prop.imageGallery) ? prop.imageGallery[0]?.url : prop.imageGallery) ||
    prop.image ||
    "";
...
export async function fetchLocalities(city) {
  ...
  try {
    ...
    return (res?.data?.data || []).map((x) => ({ name: x.name, id: x._id })).filter((x) => x.name);
  } catch (error) {
