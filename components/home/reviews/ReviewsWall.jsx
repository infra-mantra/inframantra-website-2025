...
function renderText(text) {
  ...
  return parts.map((p, i) =>
    i % 2 === 1 ? (
      <span key={i} className={styles.rwHl}>
        {p}
      </span>
    ) : (
...
function RatingStars({ value = 4.4 }) {
  ...
  return (
    <span className={styles.rwRatingStars} role="img" aria-label={`${value} out of 5 stars`}>
  ...
  return (
    <article className={styles.rwCard}>
      <div className={styles.rwCardHead}>
        <span className={styles.rwQuote} aria-hidden="true">
          &#8220;
        </span>
