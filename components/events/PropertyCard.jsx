...
import { PROJECTS } from "../nri/ProjectSlider.jsx";
    ...
    <div className={styles.r_card_pro}>
      <div className={styles.r_imageSlider}>
        {project.images.map((src, i) => (
          <div key={src} className={`${styles.r_slide} ${i === current ? styles.r_active : ""}`}>
        ...
        <div className={styles.r_badgeContainer}>
          {project.categories.map((cat) => (
            <span key={cat} className={styles.r_badge}>
              {cat}
            </span>
          ))}
        </div>
        ...
        <button className={`${styles.r_arrowC} ${styles.r_arrowLeft}`} onClick={prev}>
          ‹
        </button>

        <button className={`${styles.r_arrowC} ${styles.r_arrowRight}`} onClick={next}>
          ›
        </button>
      </div>
